#!/usr/bin/env python
"""
The script builds Open3D.framework for iOS.
The built framework is universal, it can be used to build app and run it on either iOS simulator or real device.

Usage:
    ./build_framework.py <outputdir>

By cmake conventions (and especially if you work with Open3D repository),
the output dir should not be a subdirectory of Open3D source tree.
...
class Builder:
    ...
    def _build(self, outdir):
        ...
        for t in alltargets:
            ...
            cmake_flags = []
            if self.contrib:
                cmake_flags.append("-DOPEN3D_EXTRA_MODULES_PATH=%s" % self.contrib)
    ...
    def getCMakeArgs(self, arch, target):
        ...
        args = [
            ...
            "-DOPEN3D_INCLUDE_INSTALL_PATH=include",
            "-DOPEN3D_3P_LIB_INSTALL_PATH=lib/3rdparty"
        ] + ([
            "-DBUILD_SHARED_LIBS=ON",
            "-DCMAKE_MACOSX_BUNDLE=ON",
            "-DCMAKE_XCODE_ATTRIBUTE_CODE_SIGNING_REQUIRED=NO",
        ] if self.dynamic else []) + ([
            "-DOPEN3D_ENABLE_NONFREE=ON"
        ] if self.enablenonfree else [])

        if len(self.exclude) > 0:
            args += ["-DBUILD_open3d_world=OFF"] if not self.dynamic else []
            args += ["-DBUILD_open3d_%s=OFF" % m for m in self.exclude]
    ...
    def getInfoPlist(self, builddirs):
        return os.path.join("platforms/", "ios", "Info.plist.in")
    ...
    def mergeLibs(self, builddir):
        res = os.path.join(builddir, "lib", "Release", "libopen3d_merged.a")
    ...
    def makeFramework(self, outdir, builddirs):
        ...
        if self.dynamic:
            ...
            libname = "open3d.framework/open3d"
        else:
            dstdir = os.path.join(framework_dir, "Versions", "A")
            libname = "libopen3d_merged.a"

        # copy headers from one of build folders
        shutil.copytree(os.path.join(builddirs[0], "install", "include", "open3d"), os.path.join(dstdir, "Headers"))
...
class iOSBuilder(Builder):
    ...
    def getToolchain(self, arch, target):
        toolchain = os.path.join("", "platforms", "ios", "cmake", "Toolchains", "Toolchain-%s_Xcode.cmake" % target)
        return toolchain
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='The script builds Open3D.framework for iOS.')
    parser.add_argument('out', metavar='OUTDIR', help='folder to put built framework')
    parser.add_argument('--open3d', metavar='DIR', default=folder, help='folder with open3d repository (default is "../.." relative to script location)')
    parser.add_argument('--contrib', metavar='DIR', default=None, help='folder with open3d_contrib repository (default is "None" - build only main framework)')
    parser.add_argument('--without', metavar='MODULE', default=[], action='append', help='Open3D modules to exclude from the framework')
    ...
    b = iOSBuilder(args.open3d, args.contrib, args.dynamic, args.bitcodedisabled, args.without, args.enablenonfree,
        [
